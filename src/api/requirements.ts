...
import { requirementsConfig } from './requirementsConfig';
...
function coreProgress(courses: Course[]): RequirementProgress {
  const config = requirementsConfig.core;
  const completed = courses.filter(
    (course) => course.categories.includes('core') && course.status === 'completed'
  );
  const completedCredits = creditSum(completed);
  const requiredCredits = config.requiredCredits;

  const missingCourses = (config.rules.find((r) => r.type === 'REQUIRED_COURSES')?.courses ?? []).filter(
    (code) => !completed.some((course) => course.courseCode === code)
  );

  const status =
    completedCredits >= requiredCredits ? 'complete' : completedCredits > 0 ? 'in_progress' : 'not_started';
...
function exploratoryProgress(courses: Course[]): RequirementProgress & { areaCredits: Record<ExploratoryArea, number> } {
  const config = requirementsConfig.exploratory;
...
function minorProgress(courses: Course[]): RequirementProgress {
  const config = requirementsConfig.minor;
...
export function findCrossListed(course: Course, allCourses: Course[]) {
  if (!course.crossListedWith || course.crossListedWith.length === 0) return [] as Course[];
  return allCourses.filter((c) => course.crossListedWith?.includes(c.courseCode));
}

export function possibleProgramsForCourse(course: Course) {
  const programs: string[] = [];
  if (course.majorTags) programs.push(...course.majorTags.map((tag) => `Major: ${tag}`));
  if (course.minorTags) programs.push(...course.minorTags.map((tag) => `Minor: ${tag}`));
  if (course.categories.includes('exploratory') && course.exploratoryArea) {
    programs.push(`Exploratory: ${course.exploratoryArea}`);
  }
